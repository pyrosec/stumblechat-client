import h264 from "h264-profile-level-id";
import { clone } from "./utils";
export const RTP_PROBATOR_MID = "probator";
export const RTP_PROBATOR_SSRC = 1234;
export const RTP_PROBATOR_CODEC_PAYLOAD_TYPE = 127;
...
export function generateProbatorRtpParameters(videoRtpParameters) {
  videoRtpParameters = clone(videoRtpParameters);
...
export function isRtxCodec(codec) {
  if (!codec) return false;
  return /.+\/rtx$/i.test(codec.mimeType);
}
export function matchCodecs(aCodec, bCodec, { strict = false, modify = false } = {}) {
...
export function matchHeaderExtensions(aExt, bExt) {
  if (aExt.kind && bExt.kind && aExt.kind !== bExt.kind) return false;
  if (aExt.uri !== bExt.uri) return false;
  return true;
}
export function reduceRtcpFeedback(codecA, codecB) {
